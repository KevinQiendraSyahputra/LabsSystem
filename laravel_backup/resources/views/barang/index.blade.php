...
@section('content')
<div class="px-3 sm:px-6 lg:px-8 py-4 sm:py-8 w-full max-w-9xl mx-auto space-y-5 sm:space-y-6" x-data="{ 
    deleteModal: false, 
    deleteActionUrl: '', 
    deleteItemName: '',
    ...
    {{-- Header Page --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-sm">
        <div class="flex items-center gap-3 sm:gap-3.5 min-w-0">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-indigo-50 text-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0 shadow-inner">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5 font-medium line-clamp-1 sm:line-clamp-none">Kelola data peralatan, aset, dan stiker QR Code</p>
            </div>
        </div>
    ...
    {{-- Search & Filter Bar --}}
    <div class="bg-white p-3.5 sm:p-5 border border-slate-200/80 rounded-2xl sm:rounded-3xl shadow-sm">
        <form action="{{ route('barang.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-2.5 sm:gap-4">
            ...
            <div class="lg:col-span-3 relative">
                ...
                <input type="search" name="search" value="{{ request('search') }}" 
                       ...
                       class="block w-full min-h-[42px] pl-10 pr-3.5 py-2.5 text-xs sm:text-sm text-slate-800 bg-slate-50/80 border border-slate-300/80 rounded-xl sm:rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white focus:outline-none transition shadow-sm" 
            ...
            <div class="lg:col-span-3 relative" x-data="{
                ...
                <button type="button" @click="open = !open" 
                        ...
                        class="w-full min-h-[42px] flex items-center justify-between bg-slate-50/80 border border-slate-300/80 text-slate-800 text-xs sm:text-sm font-semibold rounded-xl sm:rounded-2xl p-2.5 transition shadow-sm hover:bg-slate-100/70 focus:ring-2 focus:ring-indigo-500 focus:bg-white focus:outline-none">
                    <span x-text="label" class="truncate" :class="{'text-indigo-700 font-bold': selected}"></span>
                    <svg class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="{ 'rotate-180 text-indigo-600': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="sm:col-span-2 lg:col-span-2 grid grid-cols-2 gap-2">
                <button type="submit" class="w-full min-h-[42px] bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs sm:text-sm py-2.5 px-3 rounded-xl sm:rounded-2xl transition shadow-sm active:scale-[0.98] text-center flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1">
                ...
                <a href="{{ route('barang.index') }}"
                   class="w-full min-h-[42px] px-3 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs sm:text-sm py-2.5 rounded-xl sm:rounded-2xl border border-slate-300 transition active:scale-[0.98] text-center flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-1">
    ...
    <div class="grid grid-cols-1 gap-3.5 md:hidden">
        ...
        @endphp
        <div class="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-sm space-y-3 transition hover:shadow-md">
            ...
            <div class="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-xs">
                <div class="min-w-0 pr-1">
                    ...
                    <p class="text-[11px] text-slate-600 font-medium mt-1 truncate" title="{{ $barang->lokasi ?? 'Lab TKJ 1' }}">
                        <span class="text-slate-400">📍</span> {{ $barang->lokasi ?? 'Lab TKJ 1' }}
                    </p>
                </div>
    ...
    <div class="hidden md:block bg-white border border-slate-300 rounded-2xl sm:rounded-3xl shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-xs sm:text-sm text-left text-slate-600">
                <thead class="text-[10px] sm:text-xs uppercase bg-slate-900 border-b border-slate-900 font-bold tracking-wider">
                    <tr>
                        <th scope="col" class="px-4 sm:px-6 py-4 text-white font-bold">Item Barang</th>
                        <th scope="col" class="px-4 sm:px-6 py-4 text-white font-bold">Kategori & Lokasi</th>
                        <th scope="col" class="px-4 sm:px-6 py-4 text-white font-bold">Kondisi</th>
                        <th scope="col" class="px-4 sm:px-6 py-4 text-white font-bold">Stok</th>
                        <th scope="col" class="px-4 sm:px-6 py-4 text-right text-white font-bold">Aksi</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        <td class="px-4 sm:px-6 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl border border-slate-200/80 bg-slate-100 flex items-center justify-center flex-shrink-0 shadow-inner">
                                    @if($barang->foto)
                                        <img class="w-full h-full object-cover rounded-2xl" src="{{ asset('storage/' . $barang->foto) }}" alt="{{ $barang->nama_barang }}" loading="lazy">
                                    @else
                                ...
                                <div class="min-w-0">
                                    <div class="font-extrabold text-slate-900 text-xs sm:text-sm truncate">{{ $barang->nama_barang }}</div>
                                    <div class="text-[10px] sm:text-xs font-mono font-bold text-indigo-600 mt-0.5">{{ $barang->kode_barang }}</div>
                                    @if($barang->merk)
                        ...
                        <td class="px-4 sm:px-6 py-3.5 whitespace-nowrap">
                            <span class="bg-indigo-50 text-indigo-700 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border border-indigo-100 inline-block mb-1">{{ $barang->kategori }}</span>
                        ...
                        <td class="px-4 sm:px-6 py-3.5 whitespace-nowrap">
                            ...
                            @endphp
                            <span class="{{ $kBadge }} border text-[10px] sm:text-xs font-extrabold px-2.5 py-0.5 rounded-full inline-block">{{ $barang->kondisi }}</span>
                        </td>
                        <td class="px-4 sm:px-6 py-3.5 whitespace-nowrap">
                        ...
                        <td class="px-4 sm:px-6 py-3.5 whitespace-nowrap text-right font-medium">
                            <div class="flex justify-end items-center gap-1.5">
                                {{-- Detail Button --}}
                                <a href="{{ route('barang.show', $barang->id) }}" title="Detail Barang" aria-label="Detail {{ $barang->nama_barang }}" class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-600 border border-indigo-200/80 flex items-center justify-center transition active:scale-95 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                @if(Auth::check() && (Auth::user()->role === 'admin' || Auth::user()->role === 'kepala_lab'))
                                {{-- Edit Button --}}
                                <a href="{{ route('barang.edit', $barang->id) }}" title="Edit Barang" aria-label="Edit {{ $barang->nama_barang }}" class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200/80 flex items-center justify-center transition active:scale-95 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                ...
                                <button type="button" 
                                        ...
                                        title="Hapus Barang" 
                                        aria-label="Hapus {{ $barang->nama_barang }}"
                                        class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/80 flex items-center justify-center transition active:scale-95 shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
    ...
    <template x-teleport="body">
        <div x-show="deleteModal" 
             ...
             class="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-slate-900/75 backdrop-blur-sm" 
             style="display: none;"
            ...
            <div @click.away="deleteModal = false" class="bg-white rounded-3xl p-6 sm:p-7 w-full max-w-sm sm:max-w-md shadow-2xl border border-slate-100 text-center"
                ...
                {{-- Icon Alert --}}
                <div class="w-14 h-14 bg-rose-100 text-rose-600 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-rose-200/60">
                ...
                <form :action="deleteActionUrl" method="POST" class="mt-6 flex items-center justify-center gap-3">
                    ...
                    <button type="submit" class="flex-1 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs sm:text-sm shadow-md shadow-rose-600/20 transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-rose-500">
                        Ya, Hapus
                    </button>
                </form>
            </div>
