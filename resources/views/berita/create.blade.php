...
@section('page_title', 'Tulis Berita / Pengumuman')
@section('page_subtitle', 'Terbitkan pengumuman baru untuk semua atau kelas tertentu')
...
<div class="w-full flex justify-center py-4">
    <div class="w-full max-w-3xl bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200/70 overflow-hidden">
        <div class="px-5 py-2.5 sm:py-3 border-b border-slate-800 bg-slate-900 text-white">
            <h2 class="text-sm sm:text-base font-bold text-white uppercase tracking-wider">Form Tulis Pengumuman Baru</h2>
            <p class="text-xs text-slate-300 mt-0.5">Terbitkan pengumuman baru untuk pengguna atau kelas tertentu</p>
        </div>
        ...
        <div class="mb-6 flex items-center gap-2 text-sm text-slate-500 pb-3 border-b border-slate-100">
            ...
            <span class="text-slate-800 font-semibold">Tulis Pengumuman Baru</span>
        </div>
        ...
        <form action="{{ route('berita.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required
                       placeholder="Masukkan judul pengumuman praktikum..."
            ...
            {{-- Pilihan Target Kelas (Animated Dropdown) --}}
            <div class="relative" x-data="{ openTarget: false, selectedTarget: '{{ old('target_kelas', 'Semua Pengguna (Umum / Publik)') }}' }" @click.outside="openTarget = false">
            ...
            <div>
                <label for="isi" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Isi Pengumuman <span class="text-red-500">*</span></label>
                <textarea id="isi" name="isi" rows="8" required
                          placeholder="Tuliskan detail pengumuman praktikum, tata tertib, atau informasi di sini..."
                          class="w-full bg-slate-50 border @error('isi') border-red-400 bg-red-50 @else border-slate-300 @enderror rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white focus:border-transparent transition resize-none leading-relaxed">{{ old('isi') }}</textarea>
                @error('isi')
                    <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                <button type="submit"
                        class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-2.5 rounded-xl text-sm shadow-sm transition-all duration-150 active:scale-95">
