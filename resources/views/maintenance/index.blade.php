...
@section('content')
<div class="space-y-4 sm:space-y-5" x-data="maintenanceIndex()" x-init="init()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Kelola perbaikan dan perawatan alat laboratorium</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <div class="p-3.5 sm:p-4 border-b border-slate-100 bg-slate-50/60">
            <form id="filterMaintenanceForm" @submit.prevent="fetchData()" class="space-y-2.5">
                ...
                <div class="flex flex-col xs:flex-row items-stretch xs:items-center gap-2">
                    ...
                    <button type="button" @click="mobileFilterOpen = !mobileFilterOpen"
                            class="sm:hidden flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 rounded-xl text-xs font-bold text-slate-700 transition shrink-0"
                ...
                <div :class="mobileFilterOpen ? 'flex flex-col' : 'hidden sm:flex'" 
                     class="sm:flex-row gap-2.5 pt-1">
                    ...
                    @else
                        <div class="sm:w-48 relative" @click.outside="openLab = false">
                            ...
                            <div x-show="openLab"
                                 ...
                                 class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg py-1 overflow-hidden"
                                 style="display: none;"
                                 role="listbox">
        ...
        {{-- Data Container --}}
        <div id="maintenanceContainer" class="relative transition-opacity duration-150" :class="{'opacity-50 pointer-events-none': isLoading}">
            
            {{-- Loading State --}}
            <template x-if="isLoading">
                <div class="p-4 sm:p-6 space-y-3">
                    <div class="animate-pulse flex justify-between items-center gap-3">
            ...
            <div x-show="!isLoading">
                ...
                {{-- MOBILE VIEW: Compact List Layout (No Oversized Nested Cards) --}}
                <div class="block sm:hidden divide-y divide-slate-100">
                    ...
                    @empty
                        <div class="py-8 px-4 text-center">
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">Catatan perbaikan dan perawatan alat akan muncul di sini.</p>
                        </div>
                    @endforelse
                </div>
                ...
                <div class="hidden sm:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead class="bg-slate-50/80">
                            <tr>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Laboratorium</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Target / Barang</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Teknisi & Tgl</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Jenis</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Biaya</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Status</th>
                                <th scope="col" class="px-5 py-3 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-slate-100 text-xs sm:text-sm">
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                ...
                                <td class="px-5 py-3.5 whitespace-nowrap font-mono font-medium text-slate-700">
                                ...
                                <td class="px-5 py-3.5 whitespace-nowrap text-right">
                                    ...
                                    <div class="flex justify-end items-center gap-1.5">
                                        <a href="{{ route('maintenance.show', $maintenance->id) }}" 
                                           ...
                                           class="w-7 h-7 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 flex items-center justify-center transition active:scale-95"
                                           aria-label="Detail">
                                        ...
                                        @if($canManageThis)
                                            <a href="{{ route('maintenance.edit', $maintenance->id) }}" 
                                               ...
                                               class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition active:scale-95"
                                               aria-label="Edit">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            ...
                                            <button type="button" 
                                                    ...
                                                    class="w-7 h-7 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 flex items-center justify-center transition active:scale-95"
                                                    aria-label="Hapus">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <template x-teleport="body">
        <div x-show="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs" style="display: none;"
            ...
            <div @click.away="if(!isDeleting) deleteModal = false" class="bg-white rounded-2xl p-5 sm:p-6 w-full max-w-sm shadow-xl border border-slate-200 text-center"
                 x-transition:enter="transition ease-out duration-150"
                ...
                <div class="mt-5 flex items-center justify-center gap-2.5">
                    <button type="button" 
                            ...
                            class="flex-1 px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 font-bold rounded-xl text-xs sm:text-sm transition">
                        Batal
                    </button>
                    <button type="button" 
                            :disabled="isDeleting"
                            @click="submitDelete()" 
                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:bg-rose-400 text-white font-bold rounded-xl text-xs sm:text-sm shadow-xs transition active:scale-95">
