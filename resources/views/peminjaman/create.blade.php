...
<div class="max-w-3xl mx-auto space-y-4 sm:space-y-6" x-data="peminjamanCreate()">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-1">Catat data peminjaman peralatan laboratorium TKJ</p>
        </div>
        <a href="{{ route('peminjaman.index') }}" class="bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold px-4 py-2 rounded-xl text-xs transition shadow-xs">
            Kembali
        </a>
    </div>
    ...
    <div class="bg-white shadow-xs border border-slate-200/80 rounded-2xl sm:rounded-3xl overflow-hidden">
        ...
        <form action="{{ route('peminjaman.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6">
                ...
                {{-- 1. Dropdown Custom: Pilih Barang & Kategori --}}
                <div class="sm:col-span-2 relative" @click.outside="closeIf('barang')">
                    ...
                    {{-- Dropdown Container Animasi Halus --}}
                    <div x-show="openDropdown === 'barang'"
                         ...
                         class="absolute left-0 right-0 z-50 mt-1 max-h-60 bg-white border border-slate-200 rounded-2xl shadow-xl py-1 overflow-y-auto"
                         style="display: none;">
                        
                        <template x-for="b in barangsList" :key="b.id">
                            <button type="button" @click="selectBarang(b.id)"
                                    class="w-full text-left px-4 py-2.5 text-xs sm:text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 transition flex items-center justify-between border-b border-slate-50 last:border-0"
                                    :class="{'font-bold text-indigo-600 bg-indigo-50/50': selectedBarangId == b.id}">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span x-text="b.nama" class="font-bold text-slate-800"></span>
                                        <span x-show="b.kategori" x-text="b.kategori" class="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200"></span>
                                    </div>
                                    <div class="text-[11px] text-slate-400 font-mono mt-0.5" x-text="`${b.kode} • Stok Tersedia: ${b.stok_tersedia} ${b.satuan}`"></div>
                                </div>
                                <svg x-show="selectedBarangId == b.id" class="w-4 h-4 text-indigo-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                </svg>
                            </button>
                        </template>

                        <div x-show="barangsList.length === 0" class="px-4 py-3 text-center text-xs text-slate-400">
                            Tidak ada data barang yang siap dipinjam.
                        </div>
                    </div>
                    @error('barang_id')
                ...
                {{-- 2. Dropdown Multi-Select: Target Unit Barang --}}
                <div class="sm:col-span-2 relative" x-show="currentBarang && currentBarang.jumlah > 1" @click.outside="closeIf('unit')">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-[11px] text-indigo-600 font-bold" x-text="`${selectedUnits.length} unit dipilih`"></span>
                    </div>
                    ...
                    <div x-show="openDropdown === 'unit'"
                         ...
                         x-transition:leave-start="opacity-100 translate-y-0 scale-100"
                         x-transition:leave-end="opacity-0 translate-y-1 scale-98"
                         class="absolute left-0 right-0 z-50 mt-1 max-h-64 bg-white border border-slate-200 rounded-2xl shadow-xl p-3 space-y-1"
                         style="display: none;">
                        ...
                        <div class="flex items-center justify-between px-2 py-1.5 border-b border-slate-100 mb-1">
                            ...
                            <button type="button" @click.stop="resetUnits()" class="text-[11px] font-bold text-rose-600 hover:underline">Reset</button>
                        </div>
                        ...
                        <div x-show="unitsAvailable.length === 0" class="px-4 py-3 text-center text-xs text-slate-400">
                ...
                <div class="sm:col-span-2">
                    <label for="nama_peminjam" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Peminjam <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama_peminjam" id="nama_peminjam" value="{{ old('nama_peminjam') }}" required class="bg-slate-50/70 border border-slate-300 text-slate-800 text-xs sm:text-sm rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full px-4 py-2.5" placeholder="Nama lengkap siswa atau guru">
                    @error('nama_peminjam')
                        <p class="mt-1 text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Kelas / Jabatan --}}
                <div class="sm:col-span-2">
                    <label for="kelas_atau_jabatan" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kelas atau Jabatan</label>
                    <input type="text" name="kelas_atau_jabatan" id="kelas_atau_jabatan" value="{{ old('kelas_atau_jabatan') }}" class="bg-slate-50/70 border border-slate-300 text-slate-800 text-xs sm:text-sm rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full px-4 py-2.5" placeholder="Contoh: XII TKJ 1 / Guru Kejuruan TKJ">
                ...
                {{-- Jumlah Pinjam --}}
                <div>
                    <label for="jumlah_pinjam" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Jumlah Pinjam <span class="text-rose-500">*</span></label>
                    <input type="number" name="jumlah_pinjam" id="jumlah_pinjam" x-model="jumlahPinjam" min="1" :max="maxPinjam" required 
                           ...
                           class="bg-slate-50/70 border border-slate-300 text-slate-800 text-xs sm:text-sm rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full px-4 py-2.5 font-mono"
                           :class="{'bg-slate-100 cursor-not-allowed': currentBarang && currentBarang.jumlah > 1 && selectedUnits.length > 0}">
                    <span class="text-[11px] text-slate-400 mt-1 block" x-text="currentBarang ? `Maksimal: ${maxPinjam} ${currentBarang.satuan}` : ''"></span>
                    @error('jumlah_pinjam')
                        <p class="mt-1 text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" value="{{ old('tanggal_pinjam', date('Y-m-d')) }}" required class="bg-slate-50/70 border border-slate-300 text-slate-800 text-xs sm:text-sm rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full px-4 py-2.5">
                ...
                <div class="sm:col-span-2">
                    <label for="tanggal_kembali_rencana" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Rencana Tanggal Pengembalian <span class="text-rose-500">*</span></label>
                    <input type="date" name="tanggal_kembali_rencana" id="tanggal_kembali_rencana" value="{{ old('tanggal_kembali_rencana') }}" required class="bg-slate-50/70 border border-slate-300 text-slate-800 text-xs sm:text-sm rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full px-4 py-2.5">
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('peminjaman.index') }}" class="px-4 py-2.5 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 text-xs sm:text-sm font-bold transition">
                    Batal
                </a>
                <button type="submit" class="inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-xs transition active:scale-95">
    ...
    return {
        ...
        barangsList: [
            @foreach($barangs as $b)
            {
                ...
                kategori: @json($b->kategori->nama_kategori ?? ($b->kategori ?? '')),
                kode: @json($b->kode_barang),
                jumlah: {{ (int) $b->jumlah }},
                satuan: @json($b->satuan ?? 'Unit'),
                kondisi: @json($b->kondisi),
                stok_tersedia: {{ (int) ($b->stok_tersedia ?? $b->jumlah) }},
                kondisi_per_unit: {!! !empty($b->kondisi_per_unit) ? json_encode($b->kondisi_per_unit) : '{}' !!}
            },
            @endforeach
        ],
        ...
        get currentBarangLabel() {
            ...
            const kategoriText = this.currentBarang.kategori ? ` [${this.currentBarang.kategori}]` : '';
            return `${this.currentBarang.nama}${kategoriText} (${this.currentBarang.kode}) - Stok: ${this.currentBarang.stok_tersedia} ${this.currentBarang.satuan}`;
        },
