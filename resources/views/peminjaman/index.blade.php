...
@section('content')
<div class="space-y-4 sm:space-y-6" x-data="peminjamanIndexData()">
    <!-- Header -->
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl shadow-xs border border-amber-200/80 p-4 sm:p-5 flex items-center">
        ...
        <div class="bg-white rounded-2xl shadow-xs border border-slate-200/70 p-4 sm:p-5 flex items-center">
    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl shadow-xs border border-slate-200/80 relative">
        ...
        <div class="p-4 sm:p-5 border-b border-slate-200/80 bg-slate-50/50 rounded-t-2xl sm:rounded-t-3xl relative z-20">
            <form id="filterPeminjamanForm" action="{{ route('peminjaman.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" id="search" value="{{ request('search') }}" class="focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none block w-full pl-10 pr-3 py-2.5 text-xs sm:text-sm border border-slate-300 rounded-xl bg-white transition" placeholder="Cari nama peminjam...">
                    </div>
                </div>
                ...
                <div class="sm:w-56 relative" @click.outside="statusDropdownOpen = false">
                    ...
                    <button type="button" @click="statusDropdownOpen = !statusDropdownOpen"
                        ...
                        <span x-text="getStatusLabel()" :class="{'font-bold text-indigo-700': selectedStatus, 'text-slate-700': !selectedStatus}"></span>
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('peminjaman.index') }}" class="inline-flex justify-center items-center px-4 py-2.5 text-xs sm:text-sm font-semibold text-slate-500 hover:text-slate-700 bg-slate-100 rounded-xl transition">
                            Reset
                        </a>
                    @endif
                </div>
...
function peminjamanIndexData() {
    return {
        ...
        selectStatus(status) {
            ...
            document.getElementById('filterPeminjamanForm').submit();
        },

        getStatusLabel() {
            if (!this.selectedStatus) return 'Semua Status';
